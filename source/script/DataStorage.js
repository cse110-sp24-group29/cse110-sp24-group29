// OVERWRITE THIS WHEN MERGING
// THIS IS IN THE WRONG PALCE BUT I LEFT IT SINCE IT SHOULD BE ERASED IN THE MERGE
...
function saveToLocalStorage(pagesDat) {
    const data = {
        users: pagesData.users.map(user => ({
            userID: user.userID,
            name: user.name,
        ...
        currUserID: pagesData.currUser ? pagesData.currUser.userID : null,
        currProjID: pagesData.currProj ? pagesData.currProj.projID : null
    };
